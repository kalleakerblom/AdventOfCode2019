...
mod tests {
    ...
    fn day9_example_1() {
        ...
        while let Some(out) = prog.run(None) {
            println!("{:?}", out)
        }
        ...
        code.extend(iter::repeat(0).take(100));
        let mut prog = Program::new(code);
        assert_eq!(prog.run(None), Some(1_219_070_632_396_864));
        ...
        code.extend(iter::repeat(0).take(100));
        let mut prog = Program::new(code);
        assert_eq!(prog.run(None), Some(1_125_899_906_842_624));
    }
    #[test]
    fn day9_part1_and_2() {
        ...
        assert_eq!(prog.run(Some(1)), Some(3_742_852_857));
        // part 2
        let mut prog_2 = Program::new(code);
        assert_eq!(prog_2.run(Some(2)), Some(73439));
    }
}
